...
class BookingController extends Controller
{
    ...
    public function uruuNemeh(Request $request)
    {

        // $grocery = new BookingRoom();
        // $grocery->id = $request->urId;
        ...
        if($request->ajax())
        {
            ...
            // $br = BookingRoom::updateOrCreate(
            //     ['id_booking' => $request->urId],
            // );


        //     $output=$request->urId;

            // $bin = $request->bookingfrom;
            // $bout = $request->bookingto;
          
            $bookingRoom = new BookingRoom();
            $bookingRoom->id_booking=$request->urId;
            $bookingRoom->id_room=0;
            $bookingRoom->id_hotel=0;
            $bookingRoom->title='';
            $bookingRoom->num=null;
            $bookingRoom->children = 0;
            $bookingRoom->adults = 0;
            $bookingRoom->amount = 0;
            $bookingRoom->ex_tax = null;
            $bookingRoom->tax_rate =null;
            $bookingRoom->save();


            // $d =[
            //         'id_booking' => $request->$get_pm_booking_Id,
            //         'id_room' => 0,
            //         'id_hotel'=>0,
            //         'title'=>0,
            //         'num'=>0,
            ...
            // return Response::json($bookingRoom);
                
            // if($roomsi)
            // {
            //     foreach ($roomsi as $key => $room) {
            //         $output.='<tr>'.
            //         '<td style="width:100px;">
            //             <img src="https://ihotel.mn/files/img/uploads/rooms/x7aa9a44d3d01316a0b68713641741669.jpg,qw=314,ah=209,afit=crop.pagespeed.ic.LUOw-mkMNb.webp" alt="" style="width:100px;"> 
            //         </td>'.
            //         '<td><div class="rnam" data-id= '.$room->id.'>' .$room->room_type_name.'</div>('.$room->room_attribute.')</br>'.$room->add_explanation.'</td>'.
            //         '<td>'.$room->room_size.'</td>'.
            //         '<td>'.$room->room_rate.' MNT</td>'. 
            //         '<td><input class="custom-selects" type="number" name="oQ" min="0" max="3"  placeholder="0" onkeyup="this.value = minmax(this.value, 0, 3)"></td>'. 
            //         '</tr>';
            //     }
            // }

            // $output = 'hi';
            return Response($bookingRoom);
